...
!
INTEGER, DIMENSION(:,:), POINTER  &  ! number of processes to be
                             :: NBUINC           ! avoided for every budget
                                                 ! between one active
                                                 ! source to the next one
INTEGER, DIMENSION(:), POINTER    &  ! counter for all the processes
...
LOGICAL, DIMENSION(:,:,:),         & ! define the zone where the MASK
           POINTER :: LBU_MASK         ! is True
!
REAL, DIMENSION(:,:,:,:),          & ! surface for each mask at each
           POINTER :: XBUSURF          ! budget step
!
...
REAL, DIMENSION(:,:,:,:),          & ! budget arrays for RU, RV and
        POINTER :: XBURU, XBURV, XBURW ! RW (wind components) respectively
REAL, DIMENSION(:,:,:,:),          & ! budget arrays for RTH (potential
        POINTER :: XBURTH, XBURTKE     ! temperature) and RTKE (kinetic
                                           ! energy)
REAL, DIMENSION(:,:,:,:),          & ! budget arrays for RRV (water vapor)
        POINTER :: XBURRV, XBURRC      ! and RRC (cloud water)
REAL, DIMENSION(:,:,:,:),          & ! budget arrays for RRR (rain water)
        POINTER :: XBURRR, XBURRI      ! and RRI (ice)
REAL, DIMENSION(:,:,:,:),          & ! budget arrays for RRS (snow)
        POINTER :: XBURRS, XBURRG      ! and RRG (graupel)
REAL, DIMENSION(:,:,:,:),          & ! budget array for RRH (hail)
        POINTER :: XBURRH              !
REAL, DIMENSION(:,:,:,:,:), &
                 POINTER :: XBURSV       ! Budget of the SVx
REAL, DIMENSION(:,:,:),            & ! budget arrays for RHODJ at
               POINTER :: XBURHODJ , & !   scalar localization
